...
DEFAULT_RESPONSES_DIR = PROJECT_ROOT / "evals" / "outputs"
DEFAULT_MISALIGNED_QUESTIONS = DEFAULT_RESPONSES_DIR / "judged" / "misaligned_questions_4_5.jsonl"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "activations"
DEFAULT_LAYERS = [15]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Decoder block indices (0-indexed) whose resid_post to capture. Default: layer 15 only.",
    )
...
def capture_resid_post_activations(
    ...
    invalid = [idx for idx in target if idx < 0 or idx >= num_layers]
    if invalid:
        raise ValueError(
            f"Requested layer indices {invalid} are out of range for a model with {num_layers} layers."
        )
    ...
    missing = sorted(set(target) - set(activations.keys()))
    if missing:
        raise RuntimeError(
            f"Expected activations for layers {target}, but did not capture {missing}."
        )
    return activations
...
def main() -> None:
    ...
    print(f"Capturing resid_post activations for layers: {sorted(set(args.layers))}")
